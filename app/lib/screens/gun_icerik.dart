...
class GunIcerik extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final onTahmin = gun.tahminAsamasi == 'on_tahmin';
    return ListView(
      padding: const EdgeInsets.fromLTRB(12, 8, 12, 24),
      children: [
        if (onTahmin) ...[
          _OnTahminBanner(gun.sonAnaliz),
        ] else ...[
...
/// Ön tahmin aşaması: AGF henüz açıklanmamış, final analiz T-1h'ta.
class _OnTahminBanner extends StatelessWidget {
  final DateTime? sonAnaliz;
  const _OnTahminBanner(this.sonAnaliz);

  String _ik(int n) => n.toString().padLeft(2, '0');

  @override
  Widget build(BuildContext context) {
    final sonGuncelleme = sonAnaliz != null
        ? 'Son Guncelleme : ${_ik(sonAnaliz!.hour)}:${_ik(sonAnaliz!.minute)}'
        : 'Son Guncelleme : --:--';
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: HG.altin.withValues(alpha: 0.45)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.access_time, size: 16, color: HG.altin),
          const SizedBox(width: 8),
          const Text('ÖN DEĞERLENDİRME ANALİZLERİ',
              style: TextStyle(
                  fontSize: 12.5, fontWeight: FontWeight.w800, color: HG.altin,
                  letterSpacing: 0.8)),
        ]),
        const SizedBox(height: 6),
        Text(
          '$sonGuncelleme | Final Tahmini Yarıştan Yarım Saat önce yapılacaktır',
          style: const TextStyle(fontSize: 11.5, color: HG.metinSoluk, height: 1.4),
        ),
      ]),
    );
  }
