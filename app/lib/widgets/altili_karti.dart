...
class _AyakSatiri extends StatelessWidget {
  ...
  List<InlineSpan> _spanlar() {
    ...
    final seenEku = <int>{};
    final atlarList = ayak.secilenAtlar;

    if (atlarList.isNotEmpty) {
      // Ekuri duyarli: secilen_atlar uzerinden goster
      for (var i = 0; i < atlarList.length; i++) {
        final sa = atlarList[i];
        if (seenEku.contains(sa.atNo)) continue;

        if (i > 0) spans.add(const TextSpan(text: ' / '));

        final ekuPartners = sa.ekuPartnerNos;
        final kazandi = winner != null && sa.atNo == winner;

        if (ekuPartners.isNotEmpty) {
          // Ekuri: [1 AT1 / 4 AT4] (E) seklinde goster
          final ekuNos = [sa.atNo, ...ekuPartners.map((p) => p['at_no'] as int)];
          for (final p in ekuPartners) {
            seenEku.add(p['at_no'] as int);
          }
          final ekuText = ekuNos.join(' / ');
          spans.add(TextSpan(
            text: '[$ekuText] (E)',
            style: TextStyle(
              color: kazandi ? HG.yesil : HG.camgobegi,
              fontWeight: kazandi ? FontWeight.w800 : FontWeight.w600,
              fontSize: 12.5,
            ),
          ));
        } else {
          spans.add(TextSpan(
            text: kazandi ? '[${sa.atNo}]' : '${sa.atNo}',
            style: TextStyle(
      ...
      // Fallback: eski yontem (secilen int listesi)
    ...
    // Kacirdik: kazanan bizim secimimizde yok
