...
Aktif Google Play aboneliklerini periyodik olarak kontrol eder.
Süresi dolan/iptal edilen abonelikler için kullanıcıyı Standart'a düşürür.

Çalıştırma:
  cd /opt/harbi_ganyan_backend
  source .venv/bin/activate
  python cron/subscription_checker.py

Cron (örnek — her 2 saatte bir):
...
def run() -> None:
    ...
    if service is None:
        log.error("Google Play service yok — cikiyor.")
        return
    ...
    log.info(
        "Tamamlandi: kontrol=%d vip=%d dusurulen=%d hatali=%d",
