...
sys.path.insert(0, str(REPO_ROOT / "motor"))

# ganyan_master'daki normalize yardımcılarını yeniden kullan (tek kaynak).
from ganyan_master import (  # noqa: E402
    is_turkiye_hipodrom, normalize_sehir, normalize_jokey)
import re  # noqa: E402
...
def snapshot_yukle(iso: str) -> dict:
    ...
    if not f.exists():
        return {}
    try:
        # JSON anahtarları string -> kno/at_no int'e geri çevrilir.
...
def diff(eski: dict, yeni: dict) -> list[dict]:
    ...
    for hip, k_yeni in yeni.items():
        k_eski = eski.get(hip)
        if not k_eski:
            continue  # bu hipodrom üretimde yoktu -> referans yok, atla
        for kno, kv_yeni in k_yeni.items():
            ...
            for ano, av_eski in at_eski.items():
                ...
                # koşmaz: atın artık olmaması ya da aktif=False'a düşmesi
                if av_yeni is None or (av_eski.get("aktif") and not av_yeni.get("aktif")):
                    ...
                    continue
                # jokey değişikliği (eski jokey doluysa ve değiştiyse)
