...
TIER_KEYS = {"Mini 6'lı": "simitci", "Standart 6'lı": "harbi", "Geniş 6'lı": "ortakli"}
...
def parse_day_altili_txt(iso: str) -> dict[tuple[str, int], dict]:
    """Harbi_Ganyan_Analiz/<gun>/*_Altili.txt dosyasini yapisal kupona cevirir.

    Export icin birincil kaynak budur. Aksi halde gecmis gunlerde uygulama DB'si,
    dosyada yayinlanmis final kupon yerine mevcut kodla yeniden kurulan kuponu
    yazabilir.
    """
    date_dir = os.path.join(ANALIZ, iso_to_dirname(iso))
    ...
    if not os.path.isdir(date_dir):
        return out
    paths = sorted(glob.glob(os.path.join(date_dir, "*_Altili.txt")))
    for path in paths:
        current_hip = None
        current_alt = None
        current_tier = None
        for raw in open(path, encoding="utf-8", errors="replace"):
            ...
            if not line:
                continue
            m = re.search(r"🎰\s*(.*?)\s*[—-]\s*ALTILI GANYAN KUPONLARI", line)
            if m:
                current_hip = norm_hip(m.group(1))
                current_alt = None
                current_tier = None
                continue
            m = re.search(r"🎲\s*(\d+)\.\s*ALTILI GANYAN\s*\(Koşular\s*(\d+)-(\d+)\)", line)
            if m and current_hip:
                idx = int(m.group(1))
                legs = list(range(int(m.group(2)), int(m.group(3)) + 1))
                current_alt = {"idx": idx, "legs": legs, "kademeler": []}
                out[(current_hip, idx)] = current_alt
                current_tier = None
                continue
            m = re.search(r"🎲\s*(Mini 6'lı|Standart 6'lı|Geniş 6'lı)\s+([\d.,]+)\s*TL", line)
            if m and current_alt:
                ad = m.group(1)
                bedel = float(m.group(2).replace(".", "").replace(",", "."))
                current_tier = {
                    "ad": ad,
                ...
                current_alt["kademeler"].append(current_tier)
                continue
            m = re.search(r"Koşu\s+(\d+).*?\[(\d+)\s+at\]\s+([0-9()/\-]+)\s+(.*)$", line)
            if m and current_tier:
                kno = int(m.group(1))
                width = int(m.group(2))
                token = m.group(3)
                secilen = [int(x) for x in re.findall(r"\d+", token)]
                names_part = re.sub(r"\s*\(No:.*$", "", m.group(4)).strip()
                names = [x.strip() for x in names_part.split(",") if x.strip()]
                secilen_atlar = []
                for i, at_no in enumerate(secilen):
                    secilen_atlar.append({
                        "at_no": at_no,
                        "at": names[i] if i < len(names) else "",
                    })
                current_tier["ayaklar"].append({
                    "kno": kno,
                    "width": width,
                    "banko_lider": "BANKO LİDER" in line,
                    "secilen": secilen,
                    "secilen_atlar": secilen_atlar,
                })
        for alt in out.values():
            for kd in alt.get("kademeler", []):
                komb = 1
                for ay in kd.get("ayaklar", []):
                    komb *= max(1, int(ay.get("width") or len(ay.get("secilen") or [])))
                kd["komb"] = komb
    return out
...
def build_day(iso: str, ctx: dict) -> dict:
    ...
    alt_map = ctx["alt_map"]
    altili_txt_map = parse_day_altili_txt(iso)
    results = ctx["results"]
    cal = ctx["cal"]
    ...
    for hip in hips:
        ...
        for kno in knos:
            ...
            analiz_puanlari = [
                {"at_no": a["at_no"], "at": a.get("at", ""), "ana": round(a.get("ana", 0), 1)}
                for a in r.get("atlar", [])
            ]
            kosu_list.append({
                ...
                "bes": bes, "sonuc": sonuc,
                "analiz_puanlari": analiz_puanlari,
            })
        ...
        for alt in alt_map.get((iso, hip), []):
            ...
            if not ok:
                continue
            parsed_alt = altili_txt_map.get((hip, alt["idx"]))
            kademeler = []
            tier_hits = {}
            if parsed_alt and parsed_alt.get("kademeler"):
                for kd in parsed_alt.get("kademeler", []):
                    ayaklar = kd.get("ayaklar", [])
                    hits = 0
                    fin = 0
                    for p in ayaklar:
                        sec = set(p.get("secilen") or [])
                        is_fin = p["kno"] in finished
                        if is_fin and res:
                            fin += 1
                            if winning_set(res, p["kno"]) & sec:
                                hits += 1
                    key = kd.get("key") or TIER_KEYS.get(kd.get("ad"), kd.get("ad"))
                    kademeler.append({
                        "ad": kd["ad"],
                        "key": key,
                        "bedel": round(float(kd.get("bedel") or 0), 2),
                        "komb": int(kd.get("komb") or 0),
                        "ayaklar": ayaklar,
                    })
                    tier_hits[key] = hits if fin == len(ayaklar) else None
            else:
